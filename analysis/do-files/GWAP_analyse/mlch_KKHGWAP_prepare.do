...
// Geburten vorbereiten 
	do mlch_KKHGWAP_fertility_data.do
...
foreach wave of numlist 1995(1)1999 {
	...
	*keep if (Datum >= tm(1976m11) & Datum <= tm(1980m10))
	keep if (Datum >= tm(1975m11) & Datum <= tm(1980m10))
...
////////////////////////////////////////////////////////////////////////////////
		*****  REGIONALSTATISTIK EINFÜGEN  *****
////////////////////////////////////////////////////////////////////////////////
use "$temp\data_final", clear

*Regionaldaten
merge m:1 ags_clean year using ${regionalst}\exportfile_yearly_clean.dta, gen(merge_regionaldaten) // Jahresebene wird vervielfacht
/*

    Result                           # of obs.
    -----------------------------------------
    not matched                         5,309
        from master                     1,590  (merge_regionaldaten==1)
        from using                      3,719  (merge_regionaldaten==2)

    matched                             7,212  (merge_regionaldaten==3)
    -----------------------------------------
                      ...
                      |  merge_regionaldaten
    merge_kreisreform | master on  matched ( |     Total
----------------------+----------------------+----------
      master only (1) |         0      6,383 |     6,383 
       using only (2) |     1,590          0 |     1,590 
          matched (3) |         0        829 |       829 
----------------------+----------------------+----------
                Total |     1,590      7,212 |     8,802 
 



merge 1 Faelle: sind merge2 Fälle der kreisreformen (waren nicht in den KKH Diagnosedaten -> daher ags_clean == .) sollten in der Wirklichkeit nicht vorkommen
merge 2 Faelle: nur Regionalstatistik, merge_kreisreform ist missing, es gab keine Reformen und die Daten sind nicht in der KKH diagnose statistik vorhanden
*/

order ags_clean  year merge_kreisreform merge_regionaldaten
drop if ags_clean == .
********************************************************************************
//Mikrozensusgewichte ranmergen 

********************************************************************************
// andere Gewichte


order fert fertf fertm YOB MOB
