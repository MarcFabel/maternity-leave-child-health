...
\subsection{Design}
In order to estimate the causal effect of the duration of maternity leave on children's long-run health, we compare health outcomes of children whose mothers were entitled to different lengths of maternity leave. We do so by exploiting the eligibility rules that are contingent on childrens' birth date (see section \ref{sec:background}). Children born on/after a specified birth cutoff date (May 01, 1979) fall under the new regime with six months of maternity leave after childbirth, whereas children born before the threshold are exposed to two months. Assignment to treatment is a deterministic function of the birth date of the child and thus "sharp" as in the terminology of \cite{hahn2001identification}. We augment this idea of a local identification by combining the regression discontinuity design with a difference-in-difference approach.  
...
\footnote{Similar identification strategies can be found in \cite{Dustmann2012} \cite{Lalive2014}\cite{RafaelLaliveandJosefZweimuller2009}, and \cite{Ekberg2013} \cite{Huebener2017} \cite{danzer2017}.[XXX: neues paper (Chile), benutzt das auch RDDD?]}
