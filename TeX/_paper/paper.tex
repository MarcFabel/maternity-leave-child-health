...
\subsection{reform}
  \begin{itemize}
    \item description of the reform 
    \item eligibility
    \item take-up
    \item first stage
    
\newpage
 Strong effects on Maternal labor market outcomes - no effects on children's educational attainment and labor market outcomes.

% The effect of the reform on MOTHER's labor market outcomes
\textbf{The effect of the expansion in paid leave on maternal labor market outcomes}\newline 
...
\cite{Dustmann2012} and \cite{schonberg2014expansions} show that many mothers strongly adjust their labor supply downwards during the four months of extra leave, and return to the labor market as soon as the leave period terminates. Yet, it seems that there is only a small effect on long-run maternal labor supply (i.e. for the time period beyond six months after childbirth). For instance, while the reform decreased the share of mothers who had returned to the labor market by the third month after childbirth by 30.5 percentage points, the reduction in the share of returned mothers by the 52nd/76th month after childbirth is around one percentage point. Difference-in-difference estimates reveal that mothers postpone their return to work by, on average, 0.835 months in response to the change of postnatal maternity leave from two months to six months. 
%labor force participation rate


Additionally, there is an impact of the reform on available income. Despite the reduction in maternal employment, there is an overall increase in cumulative income by on average 1,700 DM as a result ofthe reform.\footnote{Maternal cumulative income is defined as the accumulated monthly income up to the point when the child is 40 years old. It consists of either monthly earnings when the mother is working, equals to the benefits when she is on leave or is zero otherwise.} This is due to the effect on mothers who would have stayed at home even without the reform and now appreciate the unexpected income. In contrast to the impact on labor supply, there is a strong effect heterogeneity in cummulative income across wages (an increase of 2,850 DM for mothers in the lowest tercile of the wage distribution, while the additional income amounts to 1,050 DM for women in the highest tercile). [XXX What was average prereform wage? set the two numbers]


    
    
\textbf{The effect of the reform in maternity leave on child outcomes}
    
    
% QUESTION: SHALL WE INCLUDE DS(2012) FIGURES, SUCH AS FIG 2A & 4A-F    
...
\begin{itemize}
	...
	\item attachment theory/Fetal origin hypothesis
