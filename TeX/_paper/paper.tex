...
%--------------------------------------------------------------------    
\bigskip
\subsection{First stage - effects of the 1979 maternity leave on other outcomes}
...
\subsection{Hospital registry}
For this research we use hospital register data spanning the period from 1995 to 2014, provided by the Research Data Centers of the Federal Statistical Office and the statistical offices of the Länder.\footnote{Due to data confidentiality regulations data access was provided via on-site and remote access at the research data center.} The register contains information on the universe of German in-patient cases (in 2017: 19.4 million observations). It covers \textit{all} patients that were discharged from \textit{any} hospital or medical precaution/rehabilitation center in Germany in the reporting year.\footnote{The data does not cover police hospitals, hospitals of the penal system, and military hospitals. Military hospitals are covered to the extent in which they offer services to civilians.\newline Medical precaution- and rehabilitation centers are included since 2003 if they have more than 100 beds. This criterion might potentially exclude establishments with a specialized range of treatments.} The data includes next to the patient's main diagnosis, socio-demographic characteristics (date of birth, gender, and the postal code of the place of residence), the length of stay, whether the patient died or underwent a surgery, and in which medicating specialist department the patient stayed the longest.\newline 

The main diagnosis is the diagnosis, which was the major reason for the patient's in-patient hospitalization. It is coded according to the guidelines of the "International Statistical Classification of Diseases and Related Health Problems" (ICD), which is maintained by the World Health Organization (WHO).\footnote{Please note that the data exploits a German modification that is issued by the German Institute of Medical Documentation (DIMDI) - a subordinate authority of the Federal Ministry of Health.} Up to and including 1999, the coding is done in the ICD-9 classification, since 2000 the ICD-10 system is in place \footnote{The numbers of the ICD classification refer to the revision, which is in place at the year of reporting. The ICD-9 classification is a 3 digit numeric code, whereas the ICD-10 is a 3 digit alphanumeric code.} With the help of the European shortlist \citep[p. 76]{statistisches2012diagnosedaten}, one can consider diagnoses across the change of the classification system. Table \ref{tab:outcomes_coding_main_chapters} gives an overview of the transformation between the two systems. The table lists the mapping provided by the European shortlist (for the main chapters and important single diagnoses) and we extend it for some subcategories.\newline

\textbf{outcomes:}
number of inpatient cases per 1,000 individuals (either births or in the less aggregated version approx. current population-explain approximation of current population)
- hospital admission is the sum of all admissions, except for diagnoses related to pregnancy, childbirth and puerperium\newline



% Vorteile data
The data set has three main advantages. First, compared to survey data, the hospital registry data covers the universe of German in-patient cases and is consequently not prone to sampling errors or problems associated with attrition. Additionally, the large number of observations come in handy when identifying local effects. Second, due to the longitudinal character of the data set we are able to trace out the trajectory of children's health differentials over 20 years of their adulthood. Third, there is almost surely no measurement error. On the one hand, this might affect the birth date, which is accurate to the month in our context. This is particularly relevant as the time of birth is the deciding factor whether an individual is assigned to the more generous leave scheme or not. \cite{Dustmann2012} do not have information about the exact birth date and have to impute the month-of-birth.\footnote{\cite{Dustmann2012} estimate that in roughly 70\% of the cases they obtain the correct birth month.} On the other hand, the lack of measurement error holds for the outcome variables, as well. As the diagnosis code matters for remuneration, the ICD code is of a very high quality. Moreover, in comparison with self-reported survey data, with administrative data there are no issues related to social desirability bias \citep{marcus2015}.\newline

%Nachteile
Nevertheless, the data set has some drawback, too. First, the source of data limits us to analyze rather extreme health events. Ideally, one could supplement health insurance data for some diagnosis types that are more likely made by practitioners, but are very likely to be affected by the extension of maternity leave (e.g. such as metabolic syndrome). Second, although the registry data is rich in both the numbers of observations and the quality of its entries, it does not offer more socio-economic variables. This implies that we can only demonstrate average effects of the 1979 maternity leave reform, but cannot present any results by subgroups. Last, connected to the previous point, we do not have information about the place of birth, implying that the location where we observe a patient in year $t$ does not necessarily have to coincide with the patient's place of birth. In order to refute this concern in the best possible way, our baseline specification is aggregated to the federal level and we rely on more disaggregated data only for the investigation of effect heterogeneity and robustness tests. In this way, 
migration within east or west Germany, 
only problematic when former Ossis immigrate to the old Länder significantly by MOB 
...
- figure \ref{fig: hospital_admissions_lengthstay_surgery} LEFT hospital admission, 
	- total typical S shape (increasing till legal age (max 19), decreasing till 26, age effect)
	- relative frequency per gender, women:relative hospital admission is monotonically decreasing  over the period 1995-2014, 1995 (16) 55\% of admission from women, 2014 (35) 48\%
	RIGHT figure: share surgery first increasing (max 22, every other admission had a surgery, afterwards stable at about 35\%) and average length of stay 7.7 days down to 6 days
...
\begin{figure}[H]\centering
	...
	\includegraphics[width=0.8\linewidth]{SOEP/Reform_shortened}
...
\end{figure}
%--------------------------------------------
% figure: hospital admission, numbers and relatuive frequency per gender
\bigskip
\begin{figure}[H]\centering
